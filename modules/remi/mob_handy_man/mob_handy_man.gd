...
func _set_difficulty(value : float):
	...
	HAND_ATTACK_DURATION_FACTOR = 1.25 / (1.0 + GlobalDifficultyParameters.FACTOR * pow(DIFFICULTY, GlobalDifficultyParameters.DELAY_EXPONENT))
	# ODILON: Make this more punitive
	# HAND_DAMAGE_PER_ATTACK = 1.0 * (1.0 + 0.8*log(1.0 + DIFFICULTY))
	HAND_DAMAGE_PER_ATTACK = round(0.8 * (1.0 + GlobalDifficultyParameters.FACTOR * pow(DIFFICULTY, GlobalDifficultyParameters.VALUE_EXPONENT)))
